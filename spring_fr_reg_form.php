...
$ptyp = $_POST['ptype'];
	//$_POST['PAYMENTREQUEST_0_AMT'] = $objUser->getTotalAmount($_POST['product']);
		...
		if($result == 1){
			...
			$sMsg = "<font color='white'>Votre paiement a été reçu avec succès.</font>";
			$msg = $objMessages->msgLayout($sMsg,$result);
			include('layouts/spring_fr_reg_form.html');
		}
		} else if($result == 3){
	...
	case "success": // success case to show the user payment got success
		echo '<title>Votre paiement a été reçu avec succès.</title>';
...
';		echo '<div class="as_wrapper">';
		echo "<h1>Votre paiement a été reçu avec succès.</h1>";
